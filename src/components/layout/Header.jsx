...
const Header = () => {
  ...
  return (
     <>
 <header className="flex justify-between w-full bg-mid items-center px-4 md:px-10 py-1 font-ebgm">
   <NavLink
   className="w-40 h-20 flex items-center focus:outline-none"
   to="/">
     <img src="/assets/Koolkicks-logo.png"
     alt="logo"
   ...
   <div className="space-x-4 flex items-center relative px-4 md:px-0">
    ...
    {!auth.user && (<div className="flex flex-col gap-1 group">
        <NavLink to="/register" className="transition-all duration-750">Register</NavLink>
        <div className='scale-0 group-hover:scale-100 bg-black h-[2px] w-full transition-all duration-300'></div>
      </div>
    )}
    {!auth.user && (
      ...
      {!auth.user || (
        <div className='relative group'>
          <Link className="inline-flex gap-x-2 w-full justify-center rounded-md bg-transparent px-3 py-1 tex-sm font-semibold text-gray-900 border-none peer">
           {FullName[0]}
           <svg
          ...
          <div className='flex-col hidden group-hover:flex absolute border-2 border-dark rounded-md z-10'>
            <NavLink className="bg-gray-100 p-2 hover:bg-brand hover:text-mid rounded-t-md"
            to={`/dashboard/${auth?.user?.role===1?"admin" : "user"}`}>
              Dashboard
              </NavLink>
              <NavLink className="bg-gray-100 p-2 hover:bg-brand hover:text-mid rounded-b-md"
      ...
      <div className='flex flex-col gap-1 group'>
        ...
        style={{marginTop : `${!auth.user ? "0.25rem" : "0.25rem"}`
        }}>
        </i> {" "}
