...
const Login = () => {
  ...
  const handleLogin = async (event)=>{
    ...
    setPassword("");
    try{
        const sendData = await axios.post(`${import.meta.env.VITE_BACKEND_API}/api/v1/auth/login`,
        { email, password });
  ...
  return (
<Layout title="Login | Top Shelf">
    <section className='py-10 w-full font-ebgm'>
    ...
    <div className="w-full rounded-lg shadow-lg md:mt-0 sm:max-w-md xl:p-0 bg-mid dark:border-gray-700">
            <div className="p-6 space-y-4 md:space-y-6 sm:p-8">
              ...
              <form className="space-y-4 md:space-y-6" onSubmit={handleLogin}>
                ...
                <button
                  type="submit"
                  className="w-full bg-brandred text-mid hover:bg-primary-700 focus:ring-4 focus:outline-none focus:ring-primary-300 rounded-lg text-sm px-5 py-2.5 text-center dark:bg-primary-600 dark:hover:bg-primary-700 dark:focus:ring-primary-800 font-semibold hover:scale-105 transition-all duration-200"
                >
                  Login
                </button>
